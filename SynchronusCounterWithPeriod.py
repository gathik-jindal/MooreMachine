...
class SynchronusCounterWithPeriod(Comb):
    ...
    def __init__(self, pydig:pd, modValue:int, period:int, clock:Clock, plot:bool = True):
        ...
        maxOutSize = SynchronusCounterWithPeriod.__bitCount(modValue)
        self.__modValue = modValue
        SynchronusCounterWithPeriod.__counter += 1

        if(period >= modValue or period <= 0):
            printErrorAndExit(f"Period {period} must be between 1 and {modValue}")

        Comb.__init__(self, func = lambda x : x, env = pydig.getEnv(), blockID = f"Mod {modValue} Period {period} Counter {SynchronusCounterWithPeriod.__counter}", 
                maxOutSize = maxOutSize, delay = 0, plot = plot, state = 0)
        
        o = pydig.combinatoricsFromObject(self)
        m = pydig.moore(plot = False, maxOutSize = maxOutSize, blockID = f"Moore {SynchronusCounterWithPeriod.__counter}")
        equal = pydig.combinatorics(maxOutSize = 1, plot = False, blockID = f"Equality Comparator {SynchronusCounterWithPeriod.__counter}", 
...
if __name__ == "__main__":
    ...
    output1 = SynchronusCounterWithPeriod(pydig, 6 , 4, clock, plot = True)
